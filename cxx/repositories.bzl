...
load("//cxx/raze:crates.bzl", "rules_rust_cxx_fetch_remote_crates")

# buildifier: disable=unnamed-macro
def rust_cxx_repositories():
    """Declare dependencies needed for cxx."""

    # nb. The cxx rule itself should work on any platform.
    _cxx_clang_repositories()

    rules_rust_cxx_fetch_remote_crates()

    native.register_toolchains(str(Label("//cxx:default_cxx_toolchain")))
...
def _cxx_clang_repositories():
    # Releases @ http://releases.llvm.org/download.html
    maybe(
        http_archive,
        name = "cxx_clang_linux",
        ...
        workspace_file_content = _COMMON_WORKSPACE.format("cxx_clang_linux"),
    )
    ...
    maybe(
        ...
        workspace_file_content = _COMMON_WORKSPACE.format("cxx_clang_osx"),
    )
